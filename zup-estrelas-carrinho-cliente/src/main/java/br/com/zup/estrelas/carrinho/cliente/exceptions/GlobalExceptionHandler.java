package br.com.zup.estrelas.carrinho.cliente.exceptions;
...
import br.com.zup.estrelas.carrinho.cliente.dto.ErrorDTO;
import br.com.zup.estrelas.carrinho.cliente.dto.ResponseDTO;
	...
	public @ResponseBody List<ErrorDTO> handlerMethodArgumentNotValidException(MethodArgumentNotValidException e) {
		...
		for (ObjectError erro : e.getBindingResult().getAllErrors()) {
			...
			if (nonNull(erro.getCodes()) && nonNull(erro.getCodes()[STR_NOME_DO_CAMPO])) {
				...
				mensagemASerExibida
						.append(nomeCampo.substring(nomeCampo.lastIndexOf(".") + IGNORA_POS_PONTO).toUpperCase());
				mensagemASerExibida.append("] - ");
